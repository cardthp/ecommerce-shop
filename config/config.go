// ข้อมูลเข้ามาที่ LoadConfig ออกเป็น interface แล้วเอาไปใช้งานกับ function ใน interface ได้เลยและอิงตาม interface type ที่ใช้ได้
// 1.สร้าง type app เพื่อกำหนด datatype 2.สร้าง type config เพื่อรับต่อ
// 3.output ผ่าน IConfig แต่จะไม่สามารถนำออกมาใช้ได้เพราะ config struct เป็น private
// 4.จึงต้องสร้าง variable ที่อ่านจาก private คือ IAppConfig (ภายใน IAppConfig คือ function เอาค่ามาจากการ return)
// 5.สร้าง function return จาก IAppConfig กลับไปที่ App() เพื่อให้ IConfig ส่ง output ออกไปได้
// เพิ่มเติม การส่ง IAppConfig ไป IConfig ต้องมีฟังก์ชั่นเพิ่มซึ่งส่งตรงๆไม่ได้
...
func LoadConfig(path string) IConfig {
	...
	return &config{
		app: &app{
			host: envMap["APP_HOST"], //string can use this form
			port: func() int {
				...
				return p
			}(), //int use this form to catch error
			name:    envMap["APP_NAME"],
			version: envMap["APP_VERSION"],
			readTimeout: func() time.Duration {
				t, err := strconv.Atoi(envMap["APP_READ_TIMEOUT"]) //Atoi is convert string to int
				if err != nil {
					log.Fatalf("load read timeout failed: %v", err)
				}
				return time.Duration(int64(t) * int64(math.Pow10(9))) // step for convert nano-second to second
			}(),
			writeTimeout: func() time.Duration {
				t, err := strconv.Atoi(envMap["APP_WRTIE_TIMEOUT"]) //Atoi is convert string to int
				if err != nil {
...
type IJwtConfig interface {
	...
	SetJwtAccessExpires(t int)  //Set Expired Date
	SetJwtRefreshExpires(t int) //Set Expired Date
}

type jwt struct {
	adminKey         string //protect from anonymous to access app
	secertKey        string
	apiKey           string
